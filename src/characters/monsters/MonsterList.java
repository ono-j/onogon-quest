...
public class MonsterList {
	...
	public enum MonsterListEnum {
		...
		private static final List<MonsterListEnum> MONSTERS = Collections.unmodifiableList(Arrays.asList(values()));
	...
	public static List<Monster> encountMonsters() {
		...
		int monsterNumber = random.nextInt(MAX_MONSTER_NUMBER) + 1;
		List<Monster> encountMonsterList = new ArrayList();
		for (int i = 0; i < monsterNumber; i++) {
			int monsterNo = MonsterList.MonsterListEnum.getRandomMonster().getId();
			switch (monsterNo) {
			case 1:
				encountMonsterList.add(new Matango(i));
				break;
			case 2:
				encountMonsterList.add(new Slime(i));
				break;
			}
		}
